...
module Events
  class MeetTheTeamTest < TestHelper
    setup do
      answer_with('Michaël')
      @event = MeetTheTeam.start
    end

    test 'create_player creates the player with given name' do
      assert_equal 'Michaël', @event.instance_variable_get(:@player).name
    end
    ...
    test 'start_the_race creates the opponent' do
      ...
      assert_equal 'Regis', @event.instance_variable_get(:@opponent).name
      assert_equal 75, @event.instance_variable_get(:@opponent).power 
    end
